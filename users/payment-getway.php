<?php 
    ...
    if(isset($_POST['amount-submit'])){
        ...
        if($amount == $total_price){
            ...
            $courier_result = mysqli_query($connect, "SELECT `id` FROM `courier` WHERE `user_id` = '$user_id' AND `status` = 0");
            $courier_id;
            while($rowdata = mysqli_fetch_array($courier_result)){
                $courier_id = $rowdata['id'];
            }
            
            $result = mysqli_query($connect, "UPDATE `price` SET `total_price`= '$total_price' WHERE `user_id` = '$user_id' AND `courier_id` = '$courier_id' ");

            if($result){

                $courier_status = mysqli_query($connect, "UPDATE `courier` SET `status`= 1 WHERE `id` = '$courier_id' AND `user_id` = '$user_id'");
